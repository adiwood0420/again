				...
				onSubmit:function(){
					...
					success:function(result){
						...
						if(result.success){
							...
							$.messager.alert("Tip","save successfully");
							...
							$.messager.alert("Tip","save failed");
...
/*     		$.post("${pageContext.request.contextPath}/leave/startApply.action",{leaveId:leaveId},function(result){
    		...
    		$.ajax({
				url:"${pageContext.request.contextPath}/leave/startApply.action",
					type:"post",
						data:{leaveId:leaveId},
							dataType:"json",
								...
								/* If json so error */
									success:function(result){
										if(result.success){
											$.messager.alert("Tip","Committed , waiting for auditing");
											$("#dg").datagrid("reload");
										}else{
											$.messager.alert("Tip","Commit failed");
										}
									},
											error:function(){
												alert("ajax error 2");
		...
		/* contentType:application/x-www-form-urlencoded;charset=utf-8 */
		/* This first is the original ajax */
...
<table id="dg" title="Leave Management" class="easyui-datagrid"
  fitColumns="true" pagination="true" rownumbers="true"
  url="${pageContext.request.contextPath}/leave/leavePage.action" fit="true" toolbar="#tb">
  
 <thead>
 	<tr>
 		<th field="ccc" checkbox="true" align="center"></th>
 		<!-- This cb field is checkbox -->
 		...
 		<!-- This action field name is whatever -->
 	</tr>
 </thead>
 	...
 	<form id="fm" method="post">
 		<table cellpadding="8px">
 			...
 			<tr>
 				...
 				<td>
 					<input type="text" name="userId" value="${currentMemberShip.userId}"/>
 					
 					<input type="text" name="state" value="未提交"/>
